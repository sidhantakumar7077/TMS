...
const AddDevotee = (props) => {
    ...
    const submitDevotee = async () => {
        ...
        try {
            const response = await axios.post(`${base_url}/api/add-devotee`, formData, {
                headers: {
            ...
            if (response.status === 200) {
                Toast.show('Devotee added successfully', Toast.LONG);
                navigation.goBack();
            } else {
                Toast.show('Failed to add devotee', Toast.LONG);
            }
        } catch (error) {
            Toast.show('Failed to add devotee', Toast.LONG);
        }
        ...
        <View style={styles.container}>
            <View style={styles.headerPart}>
                <TouchableOpacity onPress={() => navigation.goBack()} style={{ flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    <Text style={styles.headerText}>Add Devotees</Text>
                </TouchableOpacity>
            </View>
