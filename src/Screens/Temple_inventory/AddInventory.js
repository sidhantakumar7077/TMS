...
const AddInventory = (props) => {
    ...
    const submitInventory = async () => {
        var access_token = await AsyncStorage.getItem('storeAccesstoken');
        if (!item_name || !quantity || !inventory_category || !inventory_type) {
        ...
        formData.append('inventory_category', inventory_category);
        formData.append('type', inventory_type);
        ...
        try {
            const response = await axios.post(`${base_url}/api/add-inventory`, formData, {
                headers: {
            ...
            if (response.status === 200) {
                Toast.show('Inventory added successfully', Toast.LONG);
                navigation.goBack();
            } else {
                Toast.show('Failed to add Inventory', Toast.LONG);
            }
        } catch (error) {
    ...
    return (
        <View style={styles.container}>
            ...
            <ScrollView style={{ flex: 1 }}>
                ...
                {/* Submit Button */}
                <TouchableOpacity onPress={submitInventory}>
                    <LinearGradient
