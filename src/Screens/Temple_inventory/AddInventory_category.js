...
const AddInventory_category = (props) => {
    ...
    const submitCategory = async () => {
        ...
        try {
            const response = await axios.post(`${base_url}/api/add-inventory-category`, {
            ...
            if (response.status === 200) {
                Toast.show('Inventory category added successfully', Toast.LONG);
                navigation.goBack();
            } else {
                Toast.show('Failed to add inventory category', Toast.LONG);
            }
        } catch (error) {
            Toast.show('Failed to add inventory category', Toast.LONG);
        }
...
export default AddInventory_category
