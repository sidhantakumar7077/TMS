...
const AddVendors = (props) => {
    ...
    const [bankDetails, setBankDetails] = useState([{ bankName: '', accountNumber: '', ifscCode: '', upi_id: '' }]);
    ...
    const submitVendors = async () => {
        ...
        try {
            const response = await axios.post(`${base_url}/api/save-temple-vendors`, data, {
                headers: {
            ...
            if (response.status === 200) {
                Toast.show('Vendor added successfully', Toast.LONG);
                navigation.goBack();
            } else {
                Toast.show('Failed to add vendor', Toast.LONG);
            }
        } catch (error) {
            Toast.show('Failed to add vendor', Toast.LONG);
        }
        ...
        <View style={styles.container}>
            <View style={styles.headerPart}>
                <TouchableOpacity onPress={() => navigation.goBack()} style={{ flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    <Text style={styles.headerText}>Add Vendors</Text>
                </TouchableOpacity>
            </View>
